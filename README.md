...
- On first run, no workbook is loaded by default.
- After you choose a workbook in Workbook Settings, that choice is saved in user config and reused on the next app start.
- App-managed records are written starting at row 3 in the output sheet.
- Search reads from the app-managed region starting at row 3 (the first two rows in the Excel sheet are reserved for headers and other notes).
